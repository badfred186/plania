// api/process.js
// Traitement lourd : génération IA + PDF + email
// Appelé par webhook.js via HTTP interne après réponse immédiate à Stripe
...
async function htmlToPDF(htmlContent, filename) {
  try {
    ...
    console.log(`PDF.co: ${filename} (${htmlContent.length} chars)`);
    try {
      ...
      if (uploadData.url && !uploadData.error) {
        const convertRes = await fetch('https://api.pdf.co/v1/pdf/convert/from/html', {
          method: 'POST',
          headers: { 'x-api-key': apiKey, 'Content-Type': 'application/json' },
          body: JSON.stringify({ url: uploadData.url, paperSize: 'A4', orientation: 'Portrait', printBackground: true, margins: '10mm 12mm 10mm 12mm', async: false }),
        });
        const convertData = await convertRes.json();
        if (convertData.url && !convertData.error) {
          const pdfRes = await fetch(convertData.url);
          const pdfBuf = await pdfRes.arrayBuffer();
          console.log(`PDF OK via upload: ${pdfBuf.byteLength} bytes`);
          return Buffer.from(pdfBuf);
        }
      }
    } catch(e) { console.log('Upload method failed:', e.message); }
    // Fallback: HTML direct
    const convertRes2 = await fetch('https://api.pdf.co/v1/pdf/convert/from/html', {
      method: 'POST',
      headers: { 'x-api-key': apiKey, 'Content-Type': 'application/json' },
      body: JSON.stringify({ html: htmlContent, paperSize: 'A4', orientation: 'Portrait', printBackground: true, margins: '10mm 12mm 10mm 12mm', async: false }),
    });
    const cd2 = await convertRes2.json();
    if (!cd2.url || cd2.error) throw new Error('PDF failed: ' + JSON.stringify(cd2));
    const pr2 = await fetch(cd2.url);
    const pb2 = await pr2.arrayBuffer();
    console.log(`PDF OK via direct: ${pb2.byteLength} bytes`);
    return Buffer.from(pb2);
  } catch (err) {
    console.error(`htmlToPDF error (${filename}):`, err.message);
    return null;
  }
}
...
async function generateSimpleExcel(data, fin) {
  try {
    ...
    const moneyFmt = '# ##0" €";(# ##0" €");"-"';
    const pctFmt = '0.0%';
    const DARK_BLUE='0F2540', MID_BLUE='1A3A5C', GOLD_LIGHT='FFF8EC', GOLD='C8973A';
    const GREEN_L='F0FFF4', GREEN='276749', WHITE='FFFFFF', GRAY='F7FAFC';
    ...
    function setCell(ws,row,col,value,opts={}) {
      ...
      c.value=value;
      c.font={bold:opts.bold||false,size:opts.size||10,color:{argb:'FF'+(opts.color||'2D3748')},name:'Arial'};
      c.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF'+(opts.bg||WHITE)}};
      c.alignment={horizontal:opts.h||'left',vertical:'middle'};
    ...
    ws.getColumn(1).width=2; ws.getColumn(2).width=38;
    ws.getColumn(3).width=18; ws.getColumn(4).width=18; ws.getColumn(5).width=18;
    ...
    sh.value='▸ CHIFFRES CLÉS';
    sh.font={bold:true,size:10,color:{argb:'FFFFFFFF'},name:'Arial'};
    sh.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF'+MID_BLUE}};
    ...
    kpis.forEach(([label,v1,v2,v3,fmt,hl],i)=>{
      const bg=hl?GOLD_LIGHT:(i%2===0?WHITE:GRAY);
      const color=hl?'8B6914':'4A5568';
      setCell(ws,r,2,label,{bold:hl,bg,color});
      [v1,v2,v3].forEach((v,j)=>setCell(ws,r,j+3,v||null,{bold:hl,bg,color,fmt,h:'right'}));
      ws.getRow(r).height=18; r++;
    });
    const buffer=await wb.xlsx.writeBuffer();
    return Buffer.from(buffer).toString('base64');
  } catch(err) { console.error('Simple Excel error:',err); return null; }
}
...
module.exports = async function handler(req, res) {
  ...
  if (!internalKey || internalKey !== process.env.STRIPE_WEBHOOK_SECRET) {
    console.error('Accès non autorisé à /api/process');
    return res.status(401).json({ error: 'Unauthorized' });
  }

  const { session, meta } = req.body;
  if (!session || !meta) return res.status(400).json({ error: 'Missing data' });

  // Répondre immédiatement puis traiter
  res.status(200).json({ processing: true });

  processPayment(session, meta).catch(err => {
    console.error('Erreur process.js:', err);
  });
};
